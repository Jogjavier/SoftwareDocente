...
export default function PorDocente() {
  ...
  useEffect(() => {
    if (docenteId) {
      ...
      axios.get(`/evaluaciones/evaluaciondepartamental/data/docente/${docenteId}`)
        .then(res => {
    ...
    <div className="min-h-screen bg-gray-50 p-6">
      ...
      <div className="bg-red-800 rounded-xl p-6 mb-8 shadow-md">
        <h1 className="text-3xl font-bold text-white text-center">
          Histórico de Evaluación Departamental por Docente
        </h1>
      </div>
      ...
      <div className="max-w-7xl mx-auto">
          ...
          {nombreDocente && (
            <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
              <p className="text-blue-800 font-medium">
                Mostrando evaluaciones departamentales de:{" "}
        ...
        {!loading && docenteId && (
          <div className="bg-white rounded-xl shadow-lg p-6 mb-8">
            <h2 className="text-xl font-bold text-gray-800 mb-4">
              Tabla de Evaluaciones Departamentales
            </h2>
                  ...
                  {evaluaciones.map(ev => {
                    ...
                    return (
                      <tr key={ev.id} className="hover:bg-gray-50">
                        ...
                        <td className={`p-3 border border-gray-300 text-center ${getColorClass(ev.docencia)}`}>
                          {parseFloat(ev.docencia).toFixed(2)}
                        </td>

                        <td className={`p-3 border border-gray-300 text-center ${getColorClass(ev.tutoria)}`}>
                          {parseFloat(ev.tutoria).toFixed(2)}
                        </td>

                        <td className={`p-3 border border-gray-300 text-center ${getColorClass(ev.vinculacion)}`}>
                          {parseFloat(ev.vinculacion).toFixed(2)}
                        </td>

                        <td className={`p-3 border border-gray-300 text-center ${getColorClass(ev.gestion)}`}>
                          {parseFloat(ev.gestion).toFixed(2)}
                        </td>

                        <td className={`p-3 border border-gray-300 text-center ${getColorClass(ev.global)}`}>
                          {parseFloat(ev.global).toFixed(2)}
                        </td>
                      </tr>
                    );
                  })}
                  ...
                  {evaluaciones.length === 0 && (
                    <tr>
                      <td colSpan="7" className="text-center p-8 text-gray-500">
                        No hay evaluaciones departamentales registradas
                      </td>
                    </tr>
                  )}
                </tbody>
